...
function drawBrandHeader(doc, { title, subtitle, generatedBy }) {
  ...
  doc.fontSize(22).fillColor(BRAND.primary).text('SocietyHub', { align: 'left' });
  doc.moveDown(0.15);
  doc.fontSize(18).fillColor(BRAND.dark).text(title, { align: 'left' });
  if (subtitle) {
    doc.moveDown(0.2);
    doc.fontSize(11).fillColor(BRAND.muted).text(subtitle);
  }
  doc.moveDown(0.5);
...
function addPageNumbers(doc) {
  ...
  for (let i = range.start; i < range.start + range.count; i += 1) {
    ...
    doc.fontSize(8).fillColor(BRAND.muted).text(
      `Page ${i - range.start + 1} of ${range.count} · Official SocietyHub document`,
...
async function generateMemberLedgerPdf(data, generatedBy = 'Cashier') {
  return createPdfBuffer((doc) => {
    ...
    doc.fontSize(9).fillColor(BRAND.muted).text(
      'This is an official SocietyHub financial statement generated for auditing and record-keeping. Deposit receipts are available individually via the cashier portal.',
      { align: 'center' }
    );
