...
loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      message.textContent = data.error || t('common.loginFailed', 'Login failed.');
      return;
    }

    window.location.href = data.user.redirectTo || ROLE_REDIRECTS[data.user.role] || '/member';
  } catch (error) {
    message.textContent = t('common.unableToLogin', 'Unable to login. Please try again.');
  }
});
...
otpRequestForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      otpMessage.textContent = data.error || t('common.unableToSendOtp', 'Unable to send OTP.');
      return;
    }
    otpMessage.textContent = data.message
      + (data.emailSent === false
        ? ' ' + t('common.emailDeliveryUnavailable', '(Email delivery unavailable — check with User Management; OTP may be in server logs.)')
        : '');
  } catch (error) {
    otpMessage.textContent = t('common.unableToRequestOtp', 'Unable to request OTP. Please try again.');
  }
});
